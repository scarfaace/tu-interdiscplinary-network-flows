...
\subsection{CIC-IDS2017 Data Set Experiments}
...
As Monday data do not have any malicious traffic, we skipped this one. All the data sets from Tuesday to Friday contain both malicious and non-malicious traffic, so we used these for the purpose of our experiments and conducted an experiment on top of each of them.

From each data set from Thursday to Friday, we derive two different data sets. One for training and evaluating a baseline model. The other one is a data set of transcriptions.

The goal for these experiments was to achieve the highest possible accuracy for the \textit{attack} class as well as for the \textit{non-attack} class. Achieving overall accuracy on top of whole test sets is meaningless for us as our data sets are imbalanced and therefore overall accuracy may distort results.\\
...
\noindent\textbf{Transcription Data Set}

We always decided to train a Random Forest on top of bag of words for the transcription data sets comparing it to a baseline Random Forest models on top of numeric vectors representing the flows using the same features as those which were used for building transcriptions.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{final-report/img/friday/friday-rf-normalized.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{final-report/img/friday/friday-baseline-normalized.png}
...
The data set contains $80\ 898$ entries out of which only 11 are attacks. We sampled $5\ 500$ non attack entries from this file for the sake of our experiment and we kept all 11 attacks. The train to test set was split in 70:30 ratio resulting in $3\ 850$ non attacks and 37 attacks in the train set and 1650 non attacks and 17 attacks in the test set.
