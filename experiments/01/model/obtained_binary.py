...
def fit_model(X_train_words, y_train, is_halving_grid_search_cv: bool):
    if is_halving_grid_search_cv:
        ...
        grid_search = HalvingGridSearchCV(base_estimator, param_grid, cv=5, factor=2, resource='n_estimators', max_resources=100, random_state=2021, n_jobs=-1, verbose=1)
...
def main():
    ...
    X_train_words, X_test_words = create_bag_of_words(X_train, X_test, commonDf)

    model = fit_model(X_train_words, y_train, is_halving_grid_search_cv)
    predictions = predict(model, X_test_words)

    print_prediction_results(model, predictions, X_test_words, y_test)
