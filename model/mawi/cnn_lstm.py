...
#%%
def get_dataframe():
    df5 = pd.read_csv("./model/mawi/full_dataset.csv")[['sourceIPAddress', 'destinationIPAddress', 'transcription', 'label']]

    dfs = [df5]
    df_temp = pd.concat(dfs)
    df_attacks = df_temp[df_temp.label == 1]
    df_non_attack = df_temp[df_temp.label == 0]
    df_non_attack_sampled = df_non_attack.sample(n=1000, random_state=123)
...
learning_rates = [0.01]  # np.linspace(0.001, 0.00000001, 20) #[1, 0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001]
accuracies = []
confusion_matrices = []
tf.random.set_seed(123)
np.random.seed(123)
...
fpr_keras, tpr_keras, thresholds_keras = roc_curve(validation_label, predictions)
