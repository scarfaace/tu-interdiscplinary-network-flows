...
#%%
# Read data
mergedAllDf = pd.read_csv("./model/mawi/full_dataset.csv", sep=',')
...
#%%
sample_weight = np.array([1000 if i == 1 else 1 for i in y_train])
...
for alg in MLA:
    ...
    # alg.fit(X_train_words, y_train, sample_weight=sample_weight)
    # y_pred = alg.predict(X_test_words)
    # score = accuracy_score(y_test, y_pred)
    # cf = confusion_matrix(y_test, y_pred)
    # f1 = f1_score(y_test, y_pred)
    scores = cross_val_score(alg, X_train_words, y_train, cv = 5, scoring = 'f1')
